...
const DrMedicalRecords = () => {
  ...
  const fetchAllRecords = async () => {
    ...
    setFetchError(null);
    try {
      const patientId = selectedPatient?.patientId || selectedPatient?.id; // ✅ reversed order
      if (!patientId) {
    ...
    return [
      ...baseFields[type].map((key) => ({
        ...
        cell: (row) => {
          ...
          if (key === "context") {
            return (
              <span className={`text-sm font-semibold px-2 py-1 rounded-full bg-${typeColors[row.context]}-100 text-${typeColors[row.context]}-800`}>
                {row.context}
              </span>
            );
          }
